...
class Anchors(object):
    def __init__(self, img_size):
        ...
        min_scale = np.array([0.04, 0.1 , 0.26])  #[0.04, 0.1 , 0.26, 0.42]
        max_scale = np.array([0.1 , 0.26, 0.42])  #[0.1 , 0.26, 0.42, 0.58]
        self.img_size = np.array([img_size[1], img_size[0]])  #array([512, 640])
        self.pyramid_levels = [3, 4, 5]
        # self.variance = [0.1, 0.2]
        self.feat_shapes = [(self.img_size + 2 ** x - 1) // (2 ** x) for x in self.pyramid_levels]
        #[array([64, 80]), array([32, 40]), array([16, 20])]

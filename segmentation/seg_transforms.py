...
class Resize(object):
    ...
    def __call__(self, img, bboxes, classes, masks):
        ...
        bboxes[:, 2]/=h
        bboxes[:, 3]/=w
        img = cv2.resize(img, dsize=(self.img_sizes))
        bboxes[:, 0]*=self.img_sizes[1]
        bboxes[:, 1]*=self.img_sizes[0]
        bboxes[:, 2]*=self.img_sizes[1]
        bboxes[:, 3]*=self.img_sizes[0]

        num_obj, h, w = masks.shape
        output_masks = np.zeros((num_obj,self.img_sizes[1], self.img_sizes[0]),dtype=masks.dtype)
        for i in range(num_obj):
            output_masks[i] = cv2.resize(masks[i,:,:],dsize=(self.img_sizes),interpolation=cv2.INTER_NEAREST)
